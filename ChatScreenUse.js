...
const ChatScreen = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch('https://9410-125-13-12-200.ngrok-free.app/get_chat_list/1', {
          method: 'GET',
        });
  ...
  const sendMessage = () => {
    if (input.trim() !== '' || image !== null) {
      ...
      setChat([...chat, { text: input, image: image, isMine: true, date: dateString }]);
      setInput('');
      setImage(null);
      const dataToSend = {
        user_id: 1,
      ...
      fetch('http://35.174.149.4/add_chat', {
        method: 'POST',
        headers: {
      ...
      <View style={styles.backLinkCollar}>
        <TouchableOpacity onPress={() => props.navigation.navigate('Map')}>
          <View style={styles.backLink}>
            ...
            <Text style={styles.backText}>運営</Text>
          </View>
        </TouchableOpacity>
      </View>
